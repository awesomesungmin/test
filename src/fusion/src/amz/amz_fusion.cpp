...
void ImageLiDARFusion::BoxCallback(const vision_msgs::msg::Detection3DArray::SharedPtr msg)
{
  ...
  if (msg->detections.size() != 0 && this->box_locker == 0)
  {
    ...
    for (size_t i = 0; i < msg->detections.size(); i++) // 길요한이 수정함
    {
...
void ImageLiDARFusion::YOLOCallback(const vision_msgs::msg::Detection2DArray::SharedPtr msg)
{
  ...
  if (msg->detections.size() != 0 && this->yolo_locker == 0)
  {
    ...
    for (size_t i = 0; i < msg->detections.size(); i++) // 길요한이 수정함
    {
      Class_name = msg->detections[i].results[0].hypothesis.class_id; // 길요한이 수정함
      ...
      Box_yolo box_yolo =
      {
        msg->detections[i].bbox.center.position.x - ((msg->detections[i].bbox.size_x) / 2) * 1.2, // x1 // 길요한이 수정함
        msg->detections[i].bbox.center.position.x + ((msg->detections[i].bbox.size_x) / 2) * 1.2, // x2 // 길요한이 수정함
        msg->detections[i].bbox.center.position.y - (msg->detections[i].bbox.size_y) / 2, // y1 // 길요한이 수정함
        msg->detections[i].bbox.center.position.y + (msg->detections[i].bbox.size_y) / 2, // y2 // 길요한이 수정함
        color
      };
...
void ImageLiDARFusion::FusionCallback()
{
  ...
  if (this->yolo_locker == 1 && this->box_locker == 1)
  {
    ...
    for (size_t i = 0; i < boxes_2d_cluster.size(); i++) // 길요한이 수정함
    {
      float max_iou = 0.0f;
      int class_id = -1;
      ...
      for (size_t j = 0; j < yolo_boxes.size(); j++) // 길요한이 수정함
      {
