...
)
{
  if(this->sync_locker == 0 && this->deli_flag == 1)
  {
    ...
    if (yolo_msg->detections.size() != 0)
    {
      ...
      {
        Class_name = yolo_msg->detections[i].results[0].hypothesis.class_id; // 클래스 가져오기
        ...
        if(yolo_msg->detections[i].bbox.size_x / yolo_msg->detections[i].bbox.size_y > 0.68)
        {
          Box_yolo box_yolo = 
          {
            yolo_msg->detections[i].bbox.center.position.x - (yolo_msg->detections[i].bbox.size_x) / 2,
            yolo_msg->detections[i].bbox.center.position.x + (yolo_msg->detections[i].bbox.size_x) / 2,
