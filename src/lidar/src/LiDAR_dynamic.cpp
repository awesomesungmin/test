#include "lidar/LiDAR_dynamic.hpp"
#include "lidar/utils.hpp"

LiDAR_dynamic::LiDAR_dynamic()
: Mission("LiDAR_dynamic")
{
  pub = this->create_publisher<sensor_msgs::msg::PointCloud2>("/Cluster/dynamic", 1);
  dynamic_stop_ = this->create_publisher<std_msgs::msg::Bool>("/LiDAR/dynamic_stop", 1); // 플래닝에게 줄 동적 stop

  RCLCPP_INFO(this->get_logger(), "LiDAR_dynamic is ready");

}

int LiDAR_dynamic::get_status_code()
{
    return 13;
}

void LiDAR_dynamic::callback(const sensor_msgs::msg::PointCloud2::SharedPtr input)
{
    if (check_pointcloud == 0)
    {
      RCLCPP_INFO(this->get_logger(), "LiDAR_dynamic node sub pointcloud");
      check_pointcloud = 1;
    }
...
void LiDAR_dynamic::find_dynamic(const pcl::PointCloud<pcl::PointXYZI> &TotalCloud)
{
